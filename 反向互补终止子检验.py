...
def reverse_complement(sequence):
    ...
    # 使用列表推导式和字典来生成反向互补序列
    reverse_complement_sequence = ''.join(complement_dict[base] for base in reversed(sequence))
...
for j in range(0, len(cols_list0)):
    ...
    filename1 = 'C:/Users/lizhouquan/Desktop/论文返工/再更新检验/反序/' + j_str + '.txt'  # 组成保存检验完成的orf的路径
    ...
    with open(filename1, "w") as d:  # 创建保存结果的txt
        ...
        for i in range(len(match_positions) - 1):
            ...
            #将filtered_sequence反向互补
            rc_sequence = reverse_complement(filtered_sequence)
            if filtered_sequence:
                results.append(rc_sequence)
        # 打印结果
        #print(results)
        n = 1
